...
def compute(options):
    ...
    if options.data_division == 'total':
        ...
        for rate in np.arange(options.min_anomaly_rate, options.max_anomaly_rate + 0.001, 0.001):
            ...
            result_file.write(
                f'anomaly rate: {rate:.3f} | precision: {evaluation[0]:.5f} | recall: {evaluation[1]:.5f} | F1-score: {evaluation[2]:.5f}\n')
        ...
        result_file.write('\nBest F1-score\n')
        result_file.write(
            f'precision: {average_eval[0]:.5f} | recall: {average_eval[1]:.5f} | F1-score: {average_eval[2]:.5f}\n\n\n')
        print('Best F1-score without point adjustment')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--smooth_scores', default=False, action='store_true',
                        help='option for smoothing scores (ewma)')
    ...
    parser.add_argument('--save_figures', default=False, action='store_true',
                        help='save figures of data and anomaly scores')
    parser.add_argument("--data_division", default='total', type=str,
                        help='data division info when saving figures; channel/class/total')
